...
class Users
{
    ...
    public function newUser($data)
    {
        ...
        $query  = "INSERT INTO tb_users (username,passwd,email) VALUES ('" . $data['subscriber'] . "','" . $password . "','" . $data['email'] . "')";
        $result = mysqli_query($this->link, $query);
        if (mysqli_affected_rows($this->link) > 0) {
            //Envia correo
            ...
            $this->enviarMail($data['email'], "Username y password", $cuerpo);
            $this->enviarMail($data['email2'], "Username y password", $cuerpo);
            return true;
        } else {
            return false;
        }
    ...
    public function getUserById($username = null)
    {
        if (!empty($username)) {
    ...
    //Verifica contraseña
    public function getPass($username = null, $password = null)
    {
        if (!empty($username)) {
            if (!empty($password)) {
                $query  = "SELECT * FROM tb_users WHERE username='" . $username . "' AND passwd='" . $password."'";
                $result = mysqli_query($this->link, $query);
                $data   = array();
                while ($data[] = mysqli_fetch_assoc($result));
                array_pop($data);
